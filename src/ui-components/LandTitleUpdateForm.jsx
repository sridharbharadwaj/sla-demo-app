...
import { getLandTitle } from "../graphql/queries";
import { updateLandTitle } from "../graphql/mutations";
const client = generateClient();
export default function LandTitleUpdateForm(props) {
  const {
    id: idProp,
    landTitle: landTitleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = landTitleRecord
      ? { ...initialValues, ...landTitleRecord }
      : initialValues;
    ...
    setPropertyAddr(cleanValues.propertyAddr);
    setErrors({});
  };
  const [landTitleRecord, setLandTitleRecord] =
    React.useState(landTitleModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getLandTitle.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getLandTitle
        : landTitleModelProp;
      setLandTitleRecord(record);
    };
    queryData();
  }, [idProp, landTitleModelProp]);
  React.useEffect(resetStateValues, [landTitleRecord]);
  const validations = {
    titleType: [],
    titleVol: [],
    titleFol: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateLandTitle.replaceAll("__typename", ""),
            variables: {
              input: {
                id: landTitleRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "LandTitleUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || landTitleModelProp)}
